...
export default class TransitInfo {
  ...
  updateVehicles(vehicles: GTFSRTFeed['vehiclePositions']) {
    ...
    vehicles.forEach((vehicle) => {
      if (vehicle.vehicle.id == '044ae755-742f-40d8-bdf1-55f1984e2ff0')
        // Ignore Q
        return
      ...
      if (vehicle.id in this.vehicles) {
        this.vehicles[vehicle.id].marker.setLngLat([vehicle.position.longitude, vehicle.position.latitude])
        this.vehicles[vehicle.id].popup.setHTML(this.makeVehiclePopup(vehicle))
      } else {
        const el = document.createElement('div')
...
`
  ...
  makeDepartureBoard(board: HAFASDepartureBoard) {
    ...
    board.Departure.forEach((departure) => {
      ...
      const rtTime = departure.rtTime
        ? new Date(Date.parse(`${departure.rtDate || departure.date}T${departure.rtTime}Z`))
        : null
